    ...
    let lastCommand = ''; // Store the last command entered
    let commandHistoryList = []; // Store all entered commands

    // Function to update and display the last command
    function updateLastCommand(command) {
        lastCommand = command;
        const lastCommandElement = document.getElementById('last-command');
        if (!lastCommandElement) {
            const newLastCommandElement = document.createElement('div');
            newLastCommandElement.id = 'last-command';
            newLastCommandElement.classList.add('text-gray-400', 'text-sm', 'mb-2', 'font-mono');
            inputSection.insertBefore(newLastCommandElement, inputSection.firstChild);
        }
        document.getElementById('last-command').textContent = lastCommand ? `Last command: ${lastCommand}` : '';
    }
    ...
    function processCommand(command, outputContainer) {
        ...
        if (command === 'clear') {
            ...
            commandHistoryList = []; // Clear command history list
            updateLastCommand(''); // Clear last command display
            return;
        }
        ...
        if (command === 'history') {
            if (commandHistoryList.length === 0) {
                outputContainer.innerHTML = `<div class="text-gray-400 font-mono">No command history available.</div>`;
                return;
            }
            const historyOutput = commandHistoryList.map((cmd, index) => `
                <div class="flex items-start">
                    <span class="text-gray-400 mr-2">${index + 1}.</span>
                    <span class="command" onclick="handleCommand('${cmd}')">${cmd}</span>
                </div>
            `).join('');
            outputContainer.innerHTML = `
                <div class="text-gray-200 font-mono mb-2">Command History:</div>
                ${historyOutput}
            `;
            return;
        }
